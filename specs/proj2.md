Zadání projektu č. 2
Předmětem projektu je vytvoření vlastní profilové webové prezentace, případně (podle vlastního uvážení) vytvoření prezentace imaginární osoby, filmové postavy apod. (v takovém případě bude konzultováno a schváleno cvičícím)

Navrhněte vhodné členění webu. Web musí mít nejméně pět hlavních informačních sekcí (např. základní údaje, vzdělání, projekty, ...), z nichž alespoň jedna musí být dále členěna na podsekce. Web bude disponovat:

Úvodní stránkou obsahující obecné informace
Další informační sekce odpovídající účelu
Vhodně navrženou globální navigací, která je stále dostupná
Může být řešeno i jako dlouhá single page.
Grafické zpracování a Kreativita
...
Důraz bude kladen na originalitu návrhu a celkový vizuální koncept prezentace. Hodnocena nebude pouze funkčnost, ale i estetická úroveň a nápaditost řešení.
Na stránkách musí být použity realistické texty odpovídající zaměření prezentace. Použití generického výplňového textu (např. Lorem Ipsum) není dovoleno. Texty by měly alespoň v základní míře simulovat reálný obsah (např. popisy projektů, profil, zkušenosti apod.).
Očekává se, že student navrhne ucelený vizuální styl (barevnost, typografie, práce s prostorem), který bude konzistentní napříč celým webem.
Významnou roli hraje také uživatelská přívětivost (UX) – přehlednost, intuitivní navigace a logické uspořádání obsahu.
Do hodnocení bude zahrnuto využití moderních prvků webového designu, například:
responzivní a adaptivní rozvržení, hamburger menu
mikrointerakce (hover efekty, animace, přechody)
vizuální zvýraznění aktivních prvků a sekcí
Kladně bude hodnocena přidaná hodnota nad rámec zadání, například:
promyšlený koncept (např. stylizace do konkrétního tématu)
pokročilejší práce s CSS (grid, flexbox, animace)
jemné interaktivní prvky pomocí JavaScriptu
Naopak negativně může být hodnoceno:
použití generických nebo šablonovitých řešení bez vlastního přínosu
nekonzistentní styl nebo chaotické rozvržení
nedostatečný kontrast a špatná čitelnost textu
...
Použití jazykových modelů a samostatnost
Použití jazykových modelů (např. generativních AI nástrojů) je povoleno a očekáváno v rozumné míře jako podpůrný nástroj při návrhu a implementaci řešení.
Student však musí plně rozumět veškerému odevzdanému kódu i obsahu a být schopen jej vysvětlit a obhájit.
Projekt musí být výsledkem vlastní práce studenta. Automaticky generované řešení bez úprav a porozumění je nepřípustné.
V případě pochybností může být student vyzván k krátké obhajobě řešení.
Výsledek projektu bude posuzován v kontextu průběžné práce studenta (zejména výsledků ze cvičení a půlsemestrálního testu). Výrazné nesrovnalosti mohou být důvodem k dalšímu ověření znalostí.
Bude přiloženo README s popisem řešení a přípdným vysvětlením, pokud bylo využito AI a jak.
Odevzdání
Projekt se odevzdává jako jeden archiv ZIP, který obsahuje všechny soubory webu. Jméno odevzdávaného souboru musí být vaslogin.zip. Tento soubor se odevzdá přes IS VUT. Pokud narazíte na limit velikosti odevzdávaného souboru (cca. 5 MB), odevzdejte v archivu pouze kód bez obrázků a dalších dat. Verzi dostupnou na WWW však samozřejmě nechte kompletní.
...
Datum odevzdání: 3. 5. 2026 v 23:59
